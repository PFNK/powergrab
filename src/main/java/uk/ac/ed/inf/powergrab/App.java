...
public class App 
{
    public static void main(String[] args) throws IOException {
		...
		if(type.equals("stateless")) {
			MapController mapC = new MapController(map, initial_position);
	    	Drone drone = new Drone(initial_position,mapC);
	    	while(drone.power > 0 && drone.moves < 250) {
	    		...
	    		System.out.printf("Current location: (%f,%f), Coins: %f, Power: %f, moved here by going: %f \n", drone.position.latitude, drone.position.longitude, drone.coins, drone.power, mapC.last_dir.name());
	    	}
			mapC.add_path();
			String json = map.features.toJson();
