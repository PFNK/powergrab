...
public class App 
{
    public static void main(String[] args) throws IOException {
    	...
    	String type = args[6];
		GameStateMap gameStateMap = new GameStateMap(date, rnd);
		...
		if(type.equals("stateless")) {
			StatelessMapController mapC = new StatelessMapController(gameStateMap, initial_position);
		...
		if(type.equals("stateful")) {
			StatefulDrone drone = new StatefulDrone(initial_position, gameStateMap, file);
