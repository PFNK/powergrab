...
/**
 * <h1>
 * Controller used by StatelessDrone, because it can't access GameMap directly.
 * </h1>
 * <p>
 ...
 * it access to the whole GameMap.
 * </p>
 */
class StatelessMapController {
    ...
    StatelessMapController(GameMap gameMap, Position position) {
    ...
    public Direction getDirectionToMove() {
        HashMap<Direction, Position> possiblePositions = gameMap.getPossiblePositions(position);
        ...
        for (Entry<Direction, Position> nextPosition : possiblePositions.entrySet()) {
            for (Feature f : gameMap.features.features()) {
                ...
                if (g.type().equals("Point")) {
                    ...
                    double distance = gameMap.calculateDistance(nextPosition.getValue(), stationPosition);
    ...
    public Direction chooseClosestStation(HashMap<String, ArrayList<Direction>> directionsToClosestStations,
        ...
        if (directionsToClosestStations.get("green").get(0) != null) {
            ...
            gameMap.updateStation(closestStations.get("greenStation"), dronesCoins, dronesPower);
            ...
            if (d == null) {
                ...
                gameMap.updateStation(closestStations.get("redStation"), dronesCoins, dronesPower);
            }
        } else {
            d = directionsToClosestStations.get("red").get(0);
            lastCollectedCoins = closestStationsCoinsPower.get("redCoins");
     ...
     * element from it using the GameMap's method which
    ...
    public Direction getRandDirection(ArrayList<Direction> directions) {
        ...
        Direction randomDirection = gameMap.getRandomDirection(directionsArray.length, directionsArray);
     ...
     * the FeatureCollection represented by GameMap.
