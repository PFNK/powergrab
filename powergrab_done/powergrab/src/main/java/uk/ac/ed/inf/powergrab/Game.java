...
public class Game implements AutoCloseable {
    ...
    public void play() throws IOException {
        if (type.equals("stateless")) {
            StatelessMapController mapC = new StatelessMapController(gameMap, initialPosition);
        ...
        if (type.equals("stateful")) {
            String file = String.format("%s-%s-%s-%s.txt", type, date[0], date[1], date[2]);
            StatefulDrone drone = new StatefulDrone(initialPosition, gameMap, file);
            while (drone.power > 1.25 && drone.movesCount < 250) {
                drone.move();
            }
            drone.addPathToMap();
