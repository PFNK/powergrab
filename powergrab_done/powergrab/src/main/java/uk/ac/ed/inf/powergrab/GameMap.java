 ...
 * It also provides useful methods that are used to
    ...
    public void updateStation(String stationId, double coins, double power) {
        for (Feature f : features.features()) {
            if (f.getProperty("id").getAsString().equals(stationId)) {
                ...
                double red_coins = f.getProperty("coins").getAsDouble();
                double red_power = f.getProperty("power").getAsDouble();

                if (coins > red_coins) f.addNumberProperty("coins", 0);
                else f.addNumberProperty("coins", red_coins + coins);

                if (power > red_power) f.addNumberProperty("power", 0);
                else f.addNumberProperty("power", red_power + power);
